...
const AUTH = {
    ...
    redirectWithToken(targetUrl) {
        ...
        // 生成临时key
        const tempKey = 'auth_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        
        // 在sessionStorage中存储认证信息
        const authData = {
            token: token,
            user: user,
            timestamp: Date.now()
        ...
        window.location.href = `${targetUrl}${separator}authKey=${tempKey}`;
    },
    ...
    receiveToken() {
        ...
        if (authKey) {
            try {
                // 尝试从sessionStorage获取认证信息
                const authDataStr = sessionStorage.getItem(authKey);
                if (authDataStr) {
            ...
            newUrl.searchParams.delete('authKey');
